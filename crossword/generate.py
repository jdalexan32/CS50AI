...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap is not None:
            ...
            for val_x in self.domains[x]:
                if not any(val_x[i] == val_y[j] for val_y in self.domains[y]):
                    to_remove.add(val_x)
    ...
    def consistent(self, assignment):
        ...
        for var1, word1 in assignment.items():
            if var1.length != len(word1) or len(set(word1)) != len(word1):
                return False

            for var2, word2 in assignment.items():
                if var1 != var2:
                    overlap = self.crossword.overlaps[var1, var2]
                    if overlap is not None and word1[overlap[0]] != word2[overlap[1]]:
                        return False
    ...
    def order_domain_values(self, var, assignment):
        ...
        return sorted(list(self.domains[var]), key=count_eliminated_values)
    ...
    def select_unassigned_variable(self, assignment):
        ...
        # First, select the variable with the fewest remaining values in its domain.
        unassigned.sort(key=lambda var: len(self.domains[var]))

        # Then, select the variable with the highest degree.
