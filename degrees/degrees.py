...
def shortest_path(source, target):
    ...
    while True:
        ...
        # Mark actor (node.state) as explored
        explored.add(node.state)

        # Find neighbors of the actor (other actors that connect via movie)
        ...
        for movie, actor in neighbors:
            if actor not in explored and not frontier.contains_state(actor):
                ...
                # Test if actor (new_node) is the target actor (goal state), if yes return path
                if new_node.state == target:
                    print(f'Solution found!', len(explored), 'actors explored to find solution.')
